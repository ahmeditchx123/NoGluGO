...
type RestOf<T extends IComment | NewComment> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...
      lastModifiedDate: comment.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...
      lastModifiedDate: restComment.lastModifiedDate ? dayjs(restComment.lastModifiedDate) : undefined,
    };
  }
