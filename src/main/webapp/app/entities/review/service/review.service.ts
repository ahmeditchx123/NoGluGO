...
type RestOf<T extends IReview | NewReview> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class ReviewService {
  ...
  getReviewIdentifier(review: Pick<IReview, 'id'>): string {
    return review.id;
  }
  ...
  protected convertDateFromClient<T extends IReview | NewReview | PartialUpdateReview>(review: T): RestOf<T> {
    return {
      ...
      lastModifiedDate: review.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReview: RestReview): IReview {
    return {
      ...
      lastModifiedDate: restReview.lastModifiedDate ? dayjs(restReview.lastModifiedDate) : undefined,
    };
  }
