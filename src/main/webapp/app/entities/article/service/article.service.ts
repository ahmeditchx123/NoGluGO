...
type RestOf<T extends IArticle | NewArticle> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class ArticleService {
  ...
  protected convertDateFromClient<T extends IArticle | NewArticle | PartialUpdateArticle>(article: T): RestOf<T> {
    return {
      ...article,
      createdDate: article.createdDate?.toJSON() ?? null,
      lastModifiedDate: article.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restArticle: RestArticle): IArticle {
    return {
      ...restArticle,
      createdDate: restArticle.createdDate ? dayjs(restArticle.createdDate) : undefined,
      lastModifiedDate: restArticle.lastModifiedDate ? dayjs(restArticle.lastModifiedDate) : undefined,
    };
  }
